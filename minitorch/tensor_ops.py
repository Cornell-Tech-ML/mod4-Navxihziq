...
def tensor_map(
    ...
    * Fill in the `out` array by applying `fn` to each
    ...
    def _map(
        ...
        # loop over all elements in out
        for ordinal in range(len(out)):
            out_index = np.zeros(len(out_shape), dtype=int)
            to_index_from_strides(ordinal, out_strides, out_index)
    ...
    * `out_shape` will be the same as `a_shape`
    ...
    def _reduce(
        ...
        for ordinal in range(len(out)):
            ...
            index = out_index.copy()
            index[reduce_dim] = 0
            accumulator = a_storage[index_to_position(index, a_strides)]

            # iterate through the reduce dimension
            for i in range(1, a_shape[reduce_dim]):
                index[reduce_dim] = i
                value = a_storage[index_to_position(index, a_strides)]
